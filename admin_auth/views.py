...
class AdminUserManagementView(APIView):
    ...
    def patch(self, request, user_id, *args, **kwargs):
        ...
        if action == 'block':
            ...
            user.is_active = False  # Optionally deactivate the user when blocked
            user.save()
            return Response({'message': f'User {user.email} has been blocked.'}, status=status.HTTP_200_OK)
        elif action == 'unblock':
            user.is_blocked = False
            user.is_active = True  # Optionally reactivate the user when unblocked
            user.save()
    ...
    def delete(self, request, user_id, *args, **kwargs):
        ...
        user.delete()
        return Response({'message': f'User {user.email} has been deleted successfully.'}, status=status.HTTP_200_OK)
